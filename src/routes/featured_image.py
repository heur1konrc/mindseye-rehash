                        ...
                        <div style="flex: 1;">
                            ...
                            <div style="margin-top: 20px; padding-top: 15px; border-top: 1px solid #333;">
                                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                                    <h4 style="color: #ff6b35; margin: 0;">Story Behind This Photo</h4>
                                    <a href="/admin/featured/story" class="btn" style="font-size: 12px; padding: 6px 12px;">Edit Story</a>
                                </div>
                                {% if featured_data.story %}
                                    <div style="background: #2a2a2a; padding: 15px; border-radius: 8px; border-left: 4px solid #ff6b35;">
                                        <p style="color: #ccc; line-height: 1.6; margin: 0;">{{ featured_data.story }}</p>
                                    </div>
                                {% else %}
                                    <div style="background: #2a2a2a; padding: 15px; border-radius: 8px; border-left: 4px solid #666;">
                                        <p style="color: #888; font-style: italic; margin: 0;">No story added yet. Click "Edit Story" to share the background behind this photograph.</p>
                                    </div>
                                {% endif %}
                            </div>
...
def set_featured_image():
    ...
    try:
        ...
        featured_data = {
            ...
            'exif_data': extract_exif_data(os.path.join(STATIC_ASSETS_DIR, selected_image['image'])),
...
@featured_bp.route('/admin/featured/story', methods=['GET', 'POST'])
def edit_featured_story():
    """Edit story for current featured image"""
    if not session.get('admin_logged_in'):
        return redirect(url_for('admin.admin_login'))
    ...
    if not featured_data:
        return redirect(url_for('featured.featured_admin', message='No featured image set', message_type='error'))
    
    if request.method == 'POST':
        try:
            # Update story
            story = request.form.get('story', '').strip()
            featured_data['story'] = story
            
            # Save updated data
            if save_featured_data(featured_data):
                return redirect(url_for('featured.featured_admin', message='Story updated successfully!', message_type='success'))
            else:
                return redirect(url_for('featured.edit_featured_story', message='Error saving story', message_type='error'))
                
        except Exception as e:
            return redirect(url_for('featured.edit_featured_story', message=f'Error: {str(e)}', message_type='error'))
    
    # GET request - show story editing form
    message = request.args.get('message', '')
    ...
    <html>
    <head>
        <title>Edit Featured Image Story</title>
        <style>
            body { 
                font-family: Arial, sans-serif; 
                background: #000; 
    ...
    <body>
        <div class="header">
            <h1>Edit Featured Image Story</h1>
            <div>
                <a href="/admin/featured" class="btn secondary">Back to Featured</a>
                ...
                <div style="flex: 1;">
                    ...
                    {% if featured_data.story %}
                        <div class="story-preview">
                            <h4>Current Story:</h4>
                            <p>{{ featured_data.story }}</p>
                        </div>
                    {% else %}
                        <div class="story-preview">
                            <h4>No Story Set</h4>
                            <p style="color: #888;">Add a story to share the background and inspiration behind this photograph.</p>
                        </div>
                    {% endif %}
                </div>
            </div>
        ...
        <div class="story-form">
            <h2 style="color: #ff6b35; margin-bottom: 20px;">Story Behind This Photo</h2>
            <form method="POST">
                <div class="form-group">
                    <label for="story">Share the story, inspiration, or background behind this photograph:</label>
                    <textarea name="story" id="story" placeholder="Tell the story behind this photo... What was happening when you took it? What inspired you? What makes this image special?">{{ featured_data.story or '' }}</textarea>
                </div>
    ...
    return render_template_string(story_edit_html, 
                                featured_data=featured_data,
                                message=message,
